...
if __name__ == "__main__":
    ...
    test_out = sys.argv[5]
    matrics_out = sys.argv[6]

    train_pre, test_pre, train_err, test_err = train(train_input, test_input, max_depth)

    train_out_file = open(train_out, "w")
    test_out_file = open(test_out, "w")
    matrics_out_file = open(matrics_out, "w")

    for i in train_pre:
        train_out_file.write(i+"\n")
    for i in test_pre:
        test_out_file.write(i+"\n")
    matrics_out_file.write("error(train): "+str(train_err)+"\n")
    matrics_out_file.write("error(test): "+str(test_err))

    train_out_file.close()
    test_out_file.close()
    matrics_out_file.close()
